...
exports.settleAllGroupDebts = settleAllGroupDebts;
const db_1 = require("../services/db");
const debtGraph_1 = require("../utils/debtGraph");
...
async function getGroups(req, res) {
    try {
        ...
        const groupMemberships = await db_1.prisma.groupMember.findMany({
            where: { userId },
            include: {
                group: {
                    include: {
...
async function joinGroupByCode(req, res) {
    try {
        ...
        // Add user as member
        const newMembership = await db_1.prisma.groupMember.create({
            data: {
                userId,
                groupId: group.id,
                role: 'MEMBER',
...
async function getGroupBalances(req, res) {
    try {
        ...
        const simplifiedTransactions = (0, debtGraph_1.simplifyDebts)(netBalances);
...
async function settleAllGroupDebts(req, res) {
    try {
        const { id } = req.params;
        const userId = req.user?.id;
        if (!userId) {
        ...
        await db_1.prisma.groupMember.updateMany({
            where: { groupId: id },
            data: { balance: 0.0 },
        });
